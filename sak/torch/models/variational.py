...
class CVAE1d(sak.torch.models.Variational):
    def __init__(self, input_channels: int = required,
        ...
        for i in range(len(self.encoder_channels)-1):
            if (i != 0):
                encoder_operations.append(sak.class_selector('torch.nn.ReLU')())
                # encoder_operations.append(sak.class_selector('torch.nn.BatchNorm1d')(self.encoder_channels[i]))
                encoder_operations.append(sak.class_selector('sak.torch.nn.Dropout1d')(0.25))

            encoder_operations.append(
                sak.class_selector(operation_encoder)(
        ...
        encoder_operations.append(sak.torch.blocks.Flatten())

        # Encoder linear mapping operations
        for i in range(len(self.encoder_linear_neurons)-1):
            encoder_operations.append(sak.class_selector('torch.nn.ReLU')())
            encoder_operations.append(sak.class_selector('sak.torch.nn.Dropout')(0.25))

            encoder_operations.append(
                sak.torch.blocks.Linear(
        ...
        ########### Bottleneck ###########
        self.bottleneck_mu = sak.torch.blocks.Linear(
            self.encoder_linear_neurons[-1], 
            self.latent_dimension, 
        )
        self.bottleneck_logvar = sak.torch.blocks.Linear(
            self.encoder_linear_neurons[-1], 
            self.latent_dimension, 
        )
        ...
        for i in range(len(self.decoder_linear_neurons)-1):
            if (i != 0):
                decoder_operations.append(sak.class_selector('torch.nn.ReLU')())
                decoder_operations.append(sak.class_selector('sak.torch.nn.Dropout')(0.25))
        ...
        decoder_operations.append(sak.torch.blocks.UnFlatten([self.decoder_channels[0],input_shape]))

        for i in range(len(self.decoder_channels)-1):
            decoder_operations.append(sak.class_selector('torch.nn.ReLU')())
            # decoder_operations.append(sak.class_selector('torch.nn.BatchNorm1d')(self.decoder_channels[i]))
            decoder_operations.append(sak.class_selector('sak.torch.nn.Dropout1d')(0.25))

            decoder_operations.append(
                sak.class_selector(operation_decoder)(
...
class VAE1d(sak.torch.models.Variational):
    ...
    def __init__(self, input_shape: int = required, encoder_neurons: list = required, 
        ...
        self.encoder_levels     = [sak.torch.blocks.Linear(self.encoder_neurons[i], self.encoder_neurons[i+1]) for i in range(len(self.encoder_neurons-1))]
        ...
        self.decoder_levels     = [sak.torch.blocks.Linear(self.decoder_neurons[i], self.decoder_neurons[i+1]) for i in range(len(self.decoder_neurons-1))]
